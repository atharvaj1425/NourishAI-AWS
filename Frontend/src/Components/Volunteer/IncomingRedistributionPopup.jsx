...
const IncomingRedistributionPopup = ({ show, onClose }) => {
  ...
  useEffect(() => {
    if (show) {
      axios.get(`${import.meta.env.VITE_BASE_URL}/api/v1/volunteers/incoming-redistributions`, {withCredentials: true})
        .then(response => {
  ...
  const handleDelivered = async (redistributionId) => {
    try {
      await axios.post(`${import.meta.env.VITE_BASE_URL}/api/v1/volunteers/update-redistribution-status/${redistributionId}/delivered`, { currentLocation }, {withCredentials: true});  
